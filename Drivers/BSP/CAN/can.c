...
CanPack can_pack;
#define MAX_RX_FIFO		100
extern uint8_t g_lwip_demo_sendbuf[LWIP_DEMO_TX_BUFSIZE];
...
uint32_t can_w = 0;
CAN_TxHeaderTypeDef g_canx_rxheader_buf[MAX_RX_FIFO];
...
uint8_t can_init(uint32_t tsjw, uint32_t tbs2, uint32_t tbs1, uint16_t brp, uint32_t mode)
{
    ...
    HAL_NVIC_SetPriority(CAN1_RX0_IRQn, 10, 0);                  /* 抢占优先级1，子优先级0 */
#endif
...
void CAN1_RX0_IRQHandler(void)
{
	...
	/*
	sprintf((char *)g_lwip_demo_sendbuf, "ID=0x%x,DLC=%d,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x\r\n", 
		(g_canx_rxheader.IDE == 0) ? g_canx_rxheader.StdId : g_canx_rxheader.ExtId, 
	g_canx_rxheader.DLC, rxbuf[0], rxbuf[1], rxbuf[2], rxbuf[3], rxbuf[4], rxbuf[5], rxbuf[6], rxbuf[7]);
	*/
	can_pack.m_id = (g_canx_rxheader.IDE == 0) ? g_canx_rxheader.StdId : g_canx_rxheader.ExtId;
	can_pack.m_len = g_canx_rxheader.DLC;
    g_lwip_send_flag |= LWIP_SEND_DATA; // 通知发送线程可以发送数据了
	
	
	#if 0
	printf("%d,%s\r\n", g_lwip_send_flag, g_lwip_demo_sendbuf);
	printf("id:%d\r\n", (g_canx_rxheader.ExtId == 0) ? g_canx_rxheader.StdId : g_canx_rxheader.ExtId);
    printf("ide:%d\r\n", g_canx_rxheader.IDE);
    printf("rtr:%d\r\n", g_canx_rxheader.RTR);
    printf("len:%d\r\n", g_canx_rxheader.DLC);
...
uint8_t can_send_msg(uint32_t id, uint8_t *msg, uint8_t len)
{
    ...
    g_canx_txheader.IDE = (id <= 0x7ff) ? CAN_ID_STD : CAN_ID_EXT;   /* 使用标准帧 */
...
uint8_t can_receive_msg(uint32_t id, uint8_t *buf)
{
    ...
    if (g_canx_rxheader.StdId!= id || g_canx_rxheader.ExtId!= id || g_canx_rxheader.IDE != CAN_ID_STD || g_canx_rxheader.RTR != CAN_RTR_DATA)       /* 接收到的ID不对 / 不是标准帧 / 不是数据帧 */
    {
        return 0;    
    }
